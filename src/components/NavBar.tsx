...
export default function NavBar() {
  ...
  return (
    <nav className={`${isMenuOpen ? 'bg-white' : ''} absolute left-0 top-0 z-50 w-screen text-white`}>
      {screenWidth > 768 ? (
        <div className='max-width-page padding-page-x mx-auto flex h-[5rem] items-center justify-between gap-16'>
          <NavLogo isMenuOpen={isMenuOpen} setIsMenuOpen={setIsMenuOpen} />
          <NavLinks isMenuOpen={isMenuOpen} />
        </div>
      ) : (
        <div
          className={`${!isMenuOpen ? '' : 'h-screen'} max-width-page padding-page-x mx-auto flex flex-col gap-4`}
        >
          <div className='flex h-[5rem] w-full items-center justify-between'>
            <NavLogo isMenuOpen={isMenuOpen} setIsMenuOpen={setIsMenuOpen} />

            <div
              className={`relative h-3 w-[1.375rem] cursor-pointer`}
              onClick={() => setIsMenuOpen(state => !state)}
            >
              {!isMenuOpen ? (
                <>
                  <span className='absolute top-0 h-0.5 w-full bg-white' />
                  <span className='absolute bottom-0 h-0.5 w-full bg-white' />
                </>
              ) : (
                <>
...
function NavLinks({ isMenuOpen }: { isMenuOpen: boolean }) {
  return (
    <ul className='maxMd:text-zinc-600 maxMd:flex-col flex gap-8'>
      {links.map(link => (
        <li
          key={link}
        ...
        >
          <p>{link}</p>
          {isMenuOpen && <IoIosArrowForward className='text-2xl' />}
        </li>
      ))}
    </ul>
  );
...
function NavLogo({ isMenuOpen, setIsMenuOpen }: NavLogoProps) {
  return (
    ...
    >
      <div className='bg-primary relative z-[6] h-[1.5rem] w-[1.5rem] rounded-full' />
